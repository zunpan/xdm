    ...
    private static List<Cell[][]> uniformTableSpan(DiffJob diffJob) {
        ...
        for (Operation op : colAddOrRemoveOps) {
            if (op.flag == OpFlag.INSERT.getVal()) {
                System.out.printf("插入：在左表第%d列后插入表头 %s , 以及数据列 %s%n", op.rowPos1, CommonUtil.cellArrToString(op.cellArr1), CommonUtil.cellArrToString(rightTableBodyT[op.rowPos2 - 1]));
            } else if (op.flag == OpFlag.REMOVE.getVal()) {
                System.out.printf("删除：删除左表的第%d列表头 %s , 以及数据列 %s%n", op.rowPos1, CommonUtil.cellArrToString(op.cellArr1), CommonUtil.cellArrToString(leftTableBodyT[op.rowPos1 - 1]));
            }
        }
        for (Operation op : colReplaceOps) {
            System.out.printf("修改：将左表表头的第%d列 %s 修改成 %s%n", op.rowPos1, CommonUtil.cellArrToString(op.cellArr1), CommonUtil.cellArrToString(op.cellArr2));
        }
    ...
    static void getRowDiffRes(Cell[][] leftTableBody, Cell[][] rightTableBody, List<Operation> rowEditMethod, int headerRowNumber) {
        ...
        for (Operation op : rowAddOrRemoveOrMoveOps) {
            if (op.flag == OpFlag.INSERT.getVal()) {
                ...
                CommonUtil.printInsertOp(op.rowPos1 + headerRowNumber, op.cellArr1, true);
            } else if (op.flag == OpFlag.REMOVE.getVal()) {
                CommonUtil.printRemoveOp(op.rowPos1 + headerRowNumber, op.cellArr1, true);
            } else if (op.flag == OpFlag.MOVE.getVal()) {
                CommonUtil.printMoveOp(op.rowPos1 + headerRowNumber, op.rowPos1New + headerRowNumber, true);
            } else if (op.flag == OpFlag.MOVE_REPLACE.getVal()) {
                CommonUtil.printMoveReplaceOp(op.rowPos1 + headerRowNumber, op.rowPos1New + headerRowNumber, op.cellArr1, op.cellArr1New, true);
            }
        }
        for (Operation op : rowReplaceOps) {
            CommonUtil.printReplaceOp(op.rowPos1 + headerRowNumber, op.cellArr1, op.cellArr2, true);
        }
    }
    ...
    private static void getMovePart(Cell[][] leftTableBody, Cell[][] rightTableBody, List<Operation> addOrRemoveOps, List<Operation> moveOps, Set<Integer> leftTableRemoveIndexSet, Set<Integer> rightTableRemoveIndexSet) {
        ...
        for (Operation op : editMethod) {
            ...
            if (op.flag == OpFlag.REPLACE.getVal() || op.flag == OpFlag.NONE.getVal()) {
                ...
                if (CommonUtil.cellArrEquals(op1.cellArr1, op2.cellArr1)) {
    ...
    private static void sortInsertAndMovePart(List<Operation> addOrRemoveOrMoveOps, List<Operation> noneOps, Cell[][] rightTableBody) {
        ...
        for (int i = 0; i < addOrRemoveOrMoveOps.size() - 1; i++) {
            ...
            for (int k = 0; k < sortRowNum; k++) {
                ...
                for (int q = i; q < j; q++) {
                    if (CommonUtil.cellArrEquals(addOrRemoveOrMoveOps.get(q).cellArr1New, waitFindRow)) {
