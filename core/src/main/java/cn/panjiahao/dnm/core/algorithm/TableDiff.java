...
public class TableDiff {
    ...
    static void getRowDiffRes(Cell[][] leftTableBody, Cell[][] rightTableBody, List<Operation> rowEditMethod, int headerRowNumber) {
        ...
        for (Operation op : rowAddOrRemoveOrMoveOps) {
            if (op.flag == OpFlag.INSERT.getVal()) {
                ...
                printInsertOp(op.rowPos1 + headerRowNumber, op.cellArr1);
            } else if (op.flag == OpFlag.REMOVE.getVal()) {
                printRemoveOp(op.rowPos1 + headerRowNumber, op.cellArr1);
            } else if (op.flag == OpFlag.MOVE.getVal()) {
                printMoveOp(op.rowPos1 + headerRowNumber, op.rowPos1New + headerRowNumber);
            } else if (op.flag == OpFlag.MOVE_REPLACE.getVal()) {
                printMoveReplaceOp(op.rowPos1 + headerRowNumber, op.rowPos1New + headerRowNumber, op.cellArr1,op.cellArr1New);
            }
        }
        for (Operation op : rowReplaceOps) {
            printReplaceOp( op.rowPos1 + headerRowNumber,op.cellArr1,op.cellArr2);
        }
    }

    private static void printInsertOp(int row, Cell[] cellArr1) {
        StringBuilder sb = new StringBuilder();
        sb.append("插入：在左表第").append(row).append("行后的");
        for (Cell cell : cellArr1) {
            sb.append("第").append(cell.getColPos()).append("列插入").append(cell.getValue()).append(",");
        }
        sb.deleteCharAt(sb.length()-1);
        System.out.println(sb.toString());
    }

    private static void printRemoveOp(int row, Cell[] cellArr1) {
        StringBuilder sb = new StringBuilder();
        sb.append("删除：删除左表的第").append(row).append("行的");
        for (Cell cell : cellArr1) {
            sb.append("第").append(cell.getColPos()).append("列的").append(cell.getValue()).append(",");
        }
        sb.deleteCharAt(sb.length()-1);
        System.out.println(sb.toString());
    }

    private static void printReplaceOp(int row, Cell[] cellArr1, Cell[] cellArr2) {
        StringBuilder sb = new StringBuilder();
        sb.append("修改：将左表的第").append(row).append("行的");
        for (int i = 0; i < cellArr1.length; i++) {
            if (!cellArr1[i].getValue().equals(cellArr2[i].getValue())) {
                sb.append("第").append(cellArr1[i].getColPos()).append("列的").append(cellArr1[i].getValue()).append("修改成").append(cellArr2[i].getValue()).append(",");
            }
        }
        sb.deleteCharAt(sb.length() - 1);
        System.out.println(sb.toString());
    ...
    private static void printMoveOp(int row, int rowNew) {
        System.out.printf("移动：将左表的第%d行移动到第%d行后面%n", row, rowNew);
    }

    private static void printMoveReplaceOp(int row, int rowNew, Cell[] cellArr1,Cell[] cellArr1New) {
        StringBuilder sb = new StringBuilder();
        sb.append("移动且修改：将左表的第").append(row).append("行移动到第").append(rowNew).append("行后面,");
        for (int i = 0; i < cellArr1.length; i++) {
            if (!cellArr1[i].getValue().equals(cellArr1New[i].getValue())) {
                sb.append("第").append(cellArr1[i].getColPos()).append("列的").append(cellArr1[i].getValue()).append("修改成").append(cellArr1New[i].getValue()).append(",");
            }
        }
        sb.deleteCharAt(sb.length() - 1);
        System.out.println(sb.toString());
