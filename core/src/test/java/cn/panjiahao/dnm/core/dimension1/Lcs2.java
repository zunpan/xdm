package cn.panjiahao.dnm.core.dimension1;
...
public class Lcs2 {
    ...
    @Test
    void lcs2Test() {
        String[] text1 = new String[]{"a", "b", "c", "b", "d", "a", "b"};
        String[] text2 = new String[]{"b", "d", "c", "a", "b", "a"};
    ...
    public static String lcs(String[] text1, String[] text2) {
        ...
        while (len1 > 0 || len2 > 0) {
            ...
            if (len1 > 0 && dp[len1][len2] == dp[len1 - 1][len2]) {
                System.out.println("删除：删除text1第" + len1 + "个字符串" + text1[len1 - 1]);
                // System.out.println(",或者给text2第"+len2+"个字符后面插入"+text1.charAt(len1-1));
                len1--;
            }
            // 向左移动
            else if (len2 > 0 && dp[len1][len2] == dp[len1][len2 - 1]) {
                System.out.println("插入：给text1第" + len1 + "个字符串后面插入" + text2[len2 - 1]);
            ...
            // 向左上方移动
                ...
                if (sb.length() < lcsLen) {
                    sb.append(text1[len1 - 1]);
                    System.out.println("对应：text1的第" + len1 + "个字符串 == text2的第" + len2 + "个字符串");
                    len1--;
                    len2--;
                }
            }
